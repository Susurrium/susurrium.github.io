...
- 当前部署仍只有手动触发；workflow 在上传产物前强制执行严格门禁，防止测试内容被误发。替换最终资料并通过严格门禁后，再经用户确认启用 `main` 自动部署和 GitHub Pages 的真实设置。
...
开发期间允许占位存在；正式发布门禁必须报告并清除。每一项的唯一替换入口和发布顺序见 [最终内容替换与 GitHub Pages 发布交接](./FINAL_RELEASE_HANDOFF.zh-CN.md)。
