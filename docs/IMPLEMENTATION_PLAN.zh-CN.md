...
> 合法状态；旧上游内容与旧聚合路由已从候选树排除并在仓库外快照中保留。候选尚未 push 或
...
状态：已完成。根入口与 Home 已分离；Blog、Trace、Saying 三个 collection、归档/详情静态路由、主导航边界和空集合行为均已建立。`verify:phase1` 已覆盖 draft 排除、路由、导航和 Blog-only 聚合边界；当前候选的旧上游内容已移出并由外部快照保留。
...
- 已建立面向最终生成 DOM/资源的生产产物审计与严格发布门禁；当前候选的渲染结果必须在新构建后通过严格门禁。旧上游内容的移除是候选树的有意边界，不等于历史重写。
