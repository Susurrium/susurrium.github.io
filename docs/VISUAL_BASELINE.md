...
| Opening Media 生命周期 | 详情页同源图片按参考站 `Hero.astro` 的 `end-0 top-4` 向右下方偏移，使用 `blur(24px)`、`rounded-3xl` 和初始 `opacity-60`；滚动经过 1/9、2/9、3/9 视口高度后依次变为 `.45/.3/.15`，ClientRouter 换页和 reduced-motion 均有清理/降级。                                                                             | Blog、Trace 和 Saying 共用 `layered-blur` 首图能力和 `projected-blur` 参考兼容变体；无自定义 mask、缩放或颜色滤镜，旧 `blur` 仅作为显式回退，应由 `verify:browser:lifecycle` 和人工滚动复核。 |
