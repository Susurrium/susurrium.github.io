...
截至当前版本，`bun run ci`、生产构建、浏览器生命周期回归和 Arthals 视觉基线采集均已通过。严格门禁仅剩以下 6 类最终内容替换项：
...
已不再有 GitHub 动态卡片 API、`target=_blank` 安全属性、Waline 或统计运行时的发布拦截。个人资料替换后应重新运行严格门禁，而不是将这些开发期例外永久白名单化。
