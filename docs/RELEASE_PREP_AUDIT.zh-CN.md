...
| owner 决定应用基线 | `70c0e97bcdeeee35d1701aa66cc03428f7d0f07d` (`docs(audit): correct final branch distance wording`)；其前整合基线为 `5fabdb5` |
...
| 既有候选封存 | `E:\\code\\blog-susurrium-release-final-20260902-043328.bundle`（历史候选证据；最终 bundle 坐标见 owner 工作单执行记录） |
...
- 5 篇当前 Saying 候选内容。出处不确定的短句在正文中保留明确说明，不把说明伪装成确定的作者事实；本轮 owner 记录确认当前候选范围。
- 生产引用的入口视频/海报、54 张 Home WebP、`tracer-companion.webp`、`social-card.webp`、实际使用的工具 SVG 和当前 favicon/头像/二维码。站长已在 owner 工作单中确认当前候选范围的公开使用权。
- `AbrilFatface-Regular.ttf` 及随附的 SIL OFL 文本、`Paralines-Regular.otf` 和当前使用字体均按 owner 记录标为已确认；来源/许可证说明仍保留在 `THIRD_PARTY_NOTICES.md`。
...
- 工作台生成的 `src/content/sayings/card-preview-saying-*.md` 与 `src/content/traces/card-preview-trace-*.md` 仅用于临时填充真实卡片；已加入根限定 `.gitignore`，并已从当前发布树移出。它们与 `draft-*` 属于可明确拒绝进入发布树的生成/草稿文件；未来新增历史候选仍必须走 owner 确认流程。
...
- 历史状态中的 86 篇 Blog、25 篇 Trace、39 篇 Saying（其中一部分是 `card-preview-*` 或 `draft-*`）曾是内容候选。站长已明确 `ALL_REJECT` 93 个真实内容路径（Blog 86、Trace 4、Saying 3），不恢复到发布树；对象仍由外部快照/bundle 保留，不等于历史脱敏或永久删除。
- `card-preview-*` 与 `draft-*` 生成/草稿文件明确拒绝进入发布树，仍以外部快照/bundle 作为可追溯证据。
...
下表记录在候选提交后、干净隔离验证 worktree 中重新执行的结果；执行前不要把历史开发期文档中的页面数量或“已通过”描述当成当前证据。验证针对最终候选树执行，精确提交/tree、审计目录和 bundle 坐标见 owner 工作单的最终执行记录。
...
| 人工/视觉页面复核 | 本地自动化与代表性人工复核 PASS；线上发布签字仍待站长 | `E:\code\blog-susurrium-visual-baseline-final-20260902-1620` 生成 112 张上下游截图（7 页 × 4 变体 × 顶/底），并人工查看 Home/About/Links 桌面与移动明色代表图；运行时异常 0。manifest 的 11 条资源错误仅为 Links 现有 `www.george-blog.top` 过期 TLS/连接关闭，和已登记的 dry-run 外部例外一致 |
