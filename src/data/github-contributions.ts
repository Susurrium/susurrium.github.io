 ...
 *
 * Directly adapted from HAN-HEATMAP / hanlife02/Astro-star at the locked
 * commit in docs/SOURCE_LEDGER.md.  The parser intentionally reads GitHub's
 * public HTML endpoint: no token, API key, server runtime or scheduled job is
 * involved.  A failed request returns null so the component can render the
 * local, non-claiming skeleton instead of breaking a static Pages build.
 */
...
/** Exported for deterministic local fixture tests; production uses it only
 * through getGitHubContributionHeatmap(). */
export function parseGitHubContributionHtml(html: string) {
  const days: ParsedGitHubContributionDay[] = []
  const dayPattern = /<td\b([^>]*)><\/td>\s*(?:<tool-tip\b[^>]*>([\s\S]*?)<\/tool-tip>)?/gi
...
export function createGitHubContributionHeatmap(
  ...
  const weekCount = Math.ceil((getUtcDayDifference(calendarStartDate, rangeEndDate) + 1) / 7)
  ...
  if (months.length > 1 && months[0].label === months[months.length - 1].label) months.shift()
...
export function createGitHubContributionHeatmapSkeleton(now = new Date()) {
  return createGitHubContributionHeatmap(new Map(), now)
}

export async function getGitHubContributionHeatmap(username: string, now = new Date()) {
  ...
  if (cachedHeatmap && cachedHeatmap.expiresAt > Date.now()) return cachedHeatmap.heatmap
  ...
  try {
    ...
    contributionCache.set(cacheKey, { expiresAt: Date.now() + CACHE_TTL, heatmap: null })
    return null
  }
}
